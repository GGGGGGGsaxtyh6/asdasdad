#!/usr/bin/env python3

from Crypto.Util.number import *
import gmpy2
from gmpy2 import mpz
import math

# Extract parameters from output.txt
N = 

cts = [, , ]

hint = 2674558878275613295915981392537201653631411909654166620884912623530781

print(f"N = {N}")
print(f"Number of ciphertexts: {len(cts)}")
print(f"Hint (sum of plaintexts): {hint}")
print(f"N bit length: {N.bit_length()}")

# Let's analyze the hint more carefully
print(f"\nHint analysis:")
print(f"Hint bit length: {hint.bit_length()}")
print(f"Hint in hex: {hex(hint)}")

# Let's try to see if the hint gives us any clues about the plaintexts
# Maybe the plaintexts are related to the hint in some way

# Let's try a different approach - maybe the plaintexts are not small integers
# but are constructed in a specific way

print(f"\nTrying to find patterns in the hint...")

# Check if hint has any special properties
print(f"Hint mod 2: {hint % 2}")
print(f"Hint mod 3: {hint % 3}")
print(f"Hint mod 5: {hint % 5}")
print(f"Hint mod 7: {hint % 7}")

# Let's try to see if we can find the plaintexts by working backwards from the hint
# If pt1 + pt2 + pt3 = hint, then pt3 = hint - pt1 - pt2
# And we need pow(pt1, e, N) = ct1, pow(pt2, e, N) = ct2, pow(pt3, e, N) = ct3

# Let's try a more systematic approach
# Maybe the plaintexts are constructed from the hint in some way

print(f"\nTrying to find plaintexts by working backwards from hint...")

# Generate all primes < 1024
def sieve(n):
    is_prime = [True] * n
    is_prime[0] = is_prime[1] = False
    for i in range(2, int(n**0.5) + 1):
        if is_prime[i]:
            for j in range(i*i, n, i):
                is_prime[j] = False
    return [i for i in range(n) if is_prime[i]]

primes = sieve(1024)
print(f"Found {len(primes)} primes < 1024")

# Try to find e by checking if any small number raised to e gives us the ciphertexts
found_solution = False
for e in primes:
    if e > 100:  # Skip very large exponents for now
        break
    
    print(f"Trying e = {e}...")
    
    # Try to find plaintexts that sum to the hint
    # Let's try all combinations of small numbers
    for pt1 in range(1, 10000):  # Increased range
        if pow(pt1, e, N) == cts[0]:
            print(f"  Found pt1 = {pt1} for ct[0]")
            for pt2 in range(1, 10000):  # Increased range
                if pow(pt2, e, N) == cts[1]:
                    print(f"  Found pt2 = {pt2} for ct[1]")
                    # Check if pt1 + pt2 + pt3 = hint for some pt3
                    remaining = hint - pt1 - pt2
                    if remaining > 0:
                        if pow(remaining, e, N) == cts[2]:
                            print(f"  Found pt3 = {remaining} for ct[2]")
                            print(f"  Sum: {pt1 + pt2 + remaining}")
                            print(f"  Hint: {hint}")
                            if pt1 + pt2 + remaining == hint:
                                print(f"  *** SOLUTION FOUND! ***")
                                print(f"  e = {e}")
                                print(f"  pt1 = {pt1}")
                                print(f"  pt2 = {pt2}")
                                print(f"  pt3 = {remaining}")
                                
                                # Convert to bytes and try to find the flag
                                try:
                                    pt1_bytes = long_to_bytes(pt1)
                                    pt2_bytes = long_to_bytes(pt2)
                                    pt3_bytes = long_to_bytes(remaining)
                                    print(f"  pt1 as bytes: {pt1_bytes}")
                                    print(f"  pt2 as bytes: {pt2_bytes}")
                                    print(f"  pt3 as bytes: {pt3_bytes}")
                                    
                                    # Try to find flag pattern
                                    all_bytes = pt1_bytes + pt2_bytes + pt3_bytes
                                    print(f"  All bytes: {all_bytes}")
                                    
                                    if b'HTB{' in all_bytes:
                                        print(f"  *** FLAG FOUND! ***")
                                        flag_start = all_bytes.find(b'HTB{')
                                        flag_end = all_bytes.find(b'}', flag_start) + 1
                                        flag = all_bytes[flag_start:flag_end]
                                        print(f"  FLAG: {flag}")
                                    
                                except:
                                    pass
                                
                                found_solution = True
                                break
            if found_solution:
                break
    if found_solution:
        break

if not found_solution:
    print("No solution found with small plaintexts")
    
    # Let's try one more approach - maybe the plaintexts are constructed differently
    print(f"\nTrying alternative approach...")
    
    # Maybe the plaintexts are not small integers but are constructed from the hint
    # Let's try to see if we can find a pattern
    
    # Check if hint can be decomposed in a meaningful way
    print(f"Hint decomposition attempts:")
    
    # Try to see if hint can be written as sum of powers
    for base in range(2, 10):
        for exp in range(2, 10):
            if base**exp <= hint:
                print(f"  {base}^{exp} = {base**exp}")
                if hint % (base**exp) == 0:
                    print(f"    Hint is divisible by {base}^{exp}")
    
    # Try to see if hint has any special structure
    hint_str = str(hint)
    print(f"Hint as string: {hint_str}")
    print(f"Hint length: {len(hint_str)}")
    
    # Check if hint can be split into meaningful parts
    for i in range(1, len(hint_str)):
        part1 = int(hint_str[:i])
        part2 = int(hint_str[i:])
        print(f"  Split at position {i}: {part1} + {part2} = {part1 + part2}")
        if part1 + part2 == hint:
            print(f"    *** HINT CAN BE SPLIT! ***")
            print(f"    Part 1: {part1}")
            print(f"    Part 2: {part2}")