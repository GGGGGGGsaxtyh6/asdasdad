#!/usr/bin/env python3

from Crypto.Util.number import *
import gmpy2
from gmpy2 import mpz
import math

# Extract parameters from output.txt
N = 

cts = [, , ]

hint = 2674558878275613295915981392537201653631411909654166620884912623530781

print(f"N = {N}")
print(f"Number of ciphertexts: {len(cts)}")
print(f"Hint (sum of plaintexts): {hint}")
print(f"N bit length: {N.bit_length()}")

# Let's try a different approach. Maybe the plaintexts are small and we can find them
# by using the hint to constrain our search

print("\nAnalyzing the hint...")
print(f"Hint bit length: {hint.bit_length()}")
print(f"Hint as hex: {hex(hint)}")

# Let's try to find the plaintexts by looking for patterns
# Since we have 3 ciphertexts and a hint, let's see if we can find them

print("\nTrying to find plaintexts by brute force with hint constraint...")

# Generate all primes < 1024
def sieve(n):
    is_prime = [True] * n
    is_prime[0] = is_prime[1] = False
    for i in range(2, int(n**0.5) + 1):
        if is_prime[i]:
            for j in range(i*i, n, i):
                is_prime[j] = False
    return [i for i in range(n) if is_prime[i]]

primes = sieve(1024)
print(f"Found {len(primes)} primes < 1024")

# Try to find e by checking if any small number raised to e gives us the ciphertexts
# But this time, let's be more systematic and check all combinations

found_solution = False
for e in primes:
    if e > 100:  # Skip very large exponents for now
        break
    
    print(f"Trying e = {e}...")
    
    # Try to find plaintexts that sum to the hint
    # Let's try all combinations of small numbers
    for pt1 in range(1, 1000):
        if pow(pt1, e, N) == cts[0]:
            print(f"  Found pt1 = {pt1} for ct[0]")
            for pt2 in range(1, 1000):
                if pow(pt2, e, N) == cts[1]:
                    print(f"  Found pt2 = {pt2} for ct[1]")
                    # Check if pt1 + pt2 + pt3 = hint for some pt3
                    remaining = hint - pt1 - pt2
                    if remaining > 0:
                        if pow(remaining, e, N) == cts[2]:
                            print(f"  Found pt3 = {remaining} for ct[2]")
                            print(f"  Sum: {pt1 + pt2 + remaining}")
                            print(f"  Hint: {hint}")
                            if pt1 + pt2 + remaining == hint:
                                print(f"  *** SOLUTION FOUND! ***")
                                print(f"  e = {e}")
                                print(f"  pt1 = {pt1}")
                                print(f"  pt2 = {pt2}")
                                print(f"  pt3 = {remaining}")
                                
                                # Convert to bytes and try to find the flag
                                try:
                                    pt1_bytes = long_to_bytes(pt1)
                                    pt2_bytes = long_to_bytes(pt2)
                                    pt3_bytes = long_to_bytes(remaining)
                                    print(f"  pt1 as bytes: {pt1_bytes}")
                                    print(f"  pt2 as bytes: {pt2_bytes}")
                                    print(f"  pt3 as bytes: {pt3_bytes}")
                                    
                                    # Try to find flag pattern
                                    all_bytes = pt1_bytes + pt2_bytes + pt3_bytes
                                    print(f"  All bytes: {all_bytes}")
                                    
                                    if b'HTB{' in all_bytes:
                                        print(f"  *** FLAG FOUND! ***")
                                        flag_start = all_bytes.find(b'HTB{')
                                        flag_end = all_bytes.find(b'}', flag_start) + 1
                                        flag = all_bytes[flag_start:flag_end]
                                        print(f"  FLAG: {flag}")
                                    
                                except:
                                    pass
                                
                                found_solution = True
                                break
            if found_solution:
                break
    if found_solution:
        break

if not found_solution:
    print("No solution found with small plaintexts")
    
    # Let's try a different approach - maybe the plaintexts are not small
    # Let's try to find them by looking for patterns in the hint
    
    print("\nTrying to find plaintexts by analyzing the hint...")
    
    # Maybe the hint contains information about the plaintexts
    # Let's try to find if the hint is a sum of specific numbers
    
    print(f"Hint: {hint}")
    print(f"Hint as hex: {hex(hint)}")
    
    # Let's try to find if the hint is a sum of powers of 2 or other patterns
    print("\nChecking if hint has special patterns...")
    
    # Check if hint is a sum of powers of 2
    binary = bin(hint)[2:]
    print(f"Hint in binary: {binary}")
    print(f"Number of 1s in binary: {binary.count('1')}")
    
    # Check if hint is a sum of powers of 10
    decimal = str(hint)
    print(f"Hint in decimal: {decimal}")
    print(f"Number of digits: {len(decimal)}")
    
    # Maybe the hint is a concatenation of smaller numbers
    print("\nChecking if hint is a concatenation...")
    for i in range(1, len(decimal) // 2):
        part1 = int(decimal[:i])
        part2 = int(decimal[i:])
        if part1 + part2 == hint:
            print(f"Hint = {part1} + {part2}")
        if part1 * part2 == hint:
            print(f"Hint = {part1} * {part2}")
    
    # Maybe the hint is a sum of three numbers
    print("\nChecking if hint is a sum of three numbers...")
    for i in range(1, len(decimal) // 3):
        for j in range(i+1, len(decimal) * 2 // 3):
            if j < len(decimal):
                part1 = int(decimal[:i])
                part2 = int(decimal[i:j])
                part3 = int(decimal[j:])
                if part1 + part2 + part3 == hint:
                    print(f"Hint = {part1} + {part2} + {part3}")
                    print(f"  part1 as bytes: {long_to_bytes(part1)}")
                    print(f"  part2 as bytes: {long_to_bytes(part2)}")
                    print(f"  part3 as bytes: {long_to_bytes(part3)}")
                    
                    # Check if any of these could be the plaintexts
                    for e in primes[:10]:  # Try first 10 primes
                        if pow(part1, e, N) == cts[0] and pow(part2, e, N) == cts[1] and pow(part3, e, N) == cts[2]:
                            print(f"  *** SOLUTION FOUND! ***")
                            print(f"  e = {e}")
                            print(f"  pt1 = {part1}")
                            print(f"  pt2 = {part2}")
                            print(f"  pt3 = {part3}")
                            found_solution = True
                            break
                    if found_solution:
                        break
            if found_solution:
                break
        if found_solution:
            break