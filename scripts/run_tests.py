...
def run_all_tests(verbose: bool = False, coverage: bool = False) -> bool:
    ...
    if coverage:
        command.extend(
            ["--cov=api_testing", "--cov=contract_testing", "--cov=integration"]
        )
...
def generate_reports() -> None:
    ...
    try:
        from ci_cd.scripts.generate_consolidated_report import \
...
def main():
    ...
    parser.add_argument(
        "--coverage", action="store_true", help="Incluir cobertura de código"
    )
    parser.add_argument(
        "--reports", action="store_true", help="Generar reportes después de los tests"
    )
    parser.add_argument(
        "--setup-only", action="store_true", help="Solo configurar entorno"
    )
