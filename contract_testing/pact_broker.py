...
class PactBrokerClient:
    ...
    def get_pact_versions(
        self,
        consumer: str,
        provider: str
    ) -> List[PactVersion]:
        """Obtener todas las versiones de un contrato"""
        url = f"{self.broker_url}/pacts/provider/{provider}/consumer/{consumer}/versions"
    ...
    def record_deployment(
        ...
        try:
            ...
            logger.info(f"Despliegue registrado: {pacticipant} v{version} en {environment}")
            return True
...
class ContractManager:
    ...
    def verify_compatibility(
        ...
        for entry in matrix:
            if (entry.get("consumer", {}).get("name") == consumer and
                entry.get("provider", {}).get("name") == provider):
