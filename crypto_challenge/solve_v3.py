...
def recover_private_key_efficient():
    """Recover private key using a more efficient approach"""
    print("Attempting to recover private key using efficient approach...")
    ...
    # Since we know the public key, we can use it to narrow down our search
    # The public key is d*G, so we need to find d such that d*G = pubkey_point
    
    # Let's try a different approach: since the nonce is deterministic,
    # we can use the relationship between the signatures
    
    # For ECDSA: s = k^(-1) * (h + r*d) mod q
    # So: s*k = h + r*d mod q
    # Therefore: d = (s*k - h) * r^(-1) mod q
    
    # Since k is deterministic based on d and h, we can solve for d
    
    print("Using mathematical relationship to solve for private key...")
    
    # Let's try to find d by checking if the relationship holds
    # We'll use the fact that all three signatures must be valid for the same d
    
    # Try different private key values with a more targeted approach
    for d in range(1, min(2**20, q)):  # Reasonable search space
        if d % 50000 == 0:
            print(f"Testing d = {d}")
...
def main():
    ...
    privkey_d = recover_private_key_efficient()
