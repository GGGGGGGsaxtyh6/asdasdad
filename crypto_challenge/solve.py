...
# import gmpy2  # Not needed for this solution
...
def recover_private_key():
    """Recover private key using the deterministic nonce vulnerability"""
    ...
    # Since nonce is deterministic, we can try to find the private key
    # by testing different values and checking if they produce the same nonces
    
    # Try different private key values
    for d in range(1, min(2**20, q)):  # Limit search space for efficiency
        if d % 10000 == 0:
            print(f"Testing d = {d}")
        ...
        # Check if this private key produces the correct signatures
        try:
            # Verify first signature
            if r1 == pow(k1, -1, q) * G:
                # Check if this d produces the correct public key
