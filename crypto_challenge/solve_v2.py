...
def recover_private_key_math():
    """Recover private key using mathematical approach"""
    print("Attempting to recover private key using mathematical approach...")
    ...
    # Since nonce is deterministic, we can use the relationship between signatures
    # For ECDSA: s = k^(-1) * (h + r*d) mod q
    # So: d = (s*k - h) * r^(-1) mod q
    
    # We need to find the private key d such that:
    # k1 = sha1(d + h1) for first signature
    # k2 = sha1(d + h2) for second signature  
    # k3 = sha1(d + h3) for third signature
    
    # And the signatures must be valid:
    # s1 = k1^(-1) * (h1 + r1*d) mod q
    # s2 = k2^(-1) * (h2 + r2*d) mod q
    # s3 = k3^(-1) * (h3 + r3*d) mod q
    
    # Let's try a more targeted search around the expected range
    # Since we have the public key, we can verify our guess
    
    print("Searching for private key...")
    
    # Try different private key values
    for d in range(1, min(2**24, q)):  # Increased search space
        if d % 100000 == 0:
            print(f"Testing d = {d}")
        ...
        try:
            ...
            if (int(test_pubkey.point.x()), int(test_pubkey.point.y())) == pubkey_point:
                ...
                # For ECDSA: s = k^(-1) * (h + r*d) mod q
                # So we need to check: s*k = h + r*d mod q
                
                # Check first signature
                if (s1 * k1) % q == (h1 + r1 * d) % q:
                    print(f"First signature verified for d = {d}")
                    # Check second signature
                    if (s2 * k2) % q == (h2 + r2 * d) % q:
                        print(f"Second signature verified for d = {d}")
...
def main():
    ...
    privkey_d = recover_private_key_math()
