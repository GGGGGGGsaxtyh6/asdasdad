...
class ConfluenceIntegration:
    ...
    def __init__(self, confluence_url: str = None, username: str = None, api_token: str = None):
        ...
        logger.info(f"ConfluenceIntegration inicializado para espacio: {self.space_key}")
    ...
    def create_test_report_page(
        ...
        try:
            ...
            # Agregar labels
            labels = ["testing", "report", report.environment, report.execution_date.strftime("%Y-%m")]
    ...
    def create_test_plan_page(
        ...
        try:
            content = self._format_test_plan_content(plan_title, plan_description, test_cases)
    ...
    def create_api_documentation_page(
        ...
        try:
            content = self._format_api_documentation_content(api_name, api_description, endpoints)
    ...
    def create_contract_documentation_page(
        ...
        try:
            content = self._format_contract_documentation_content(consumer, provider, contract_details)
        ...
        try:
            ...
            for result in results["results"]:
                pages.append({
                    ...
                    "labels": [label["name"] for label in result["content"].get("labels", [])]
                })
...
"""
    ...
    def _format_test_plan_content(self, title: str, description: str, test_cases: List[Dict[str, Any]]) -> str:
        """Formatear contenido de plan de testing"""
        test_cases_table = "|| Test Case ID || Description || Type || Priority || Status ||\n"
...
"""
    ...
    def _format_api_documentation_content(self, api_name: str, description: str, endpoints: List[Dict[str, Any]]) -> str:
...
"""
    ...
    def _format_contract_documentation_content(self, consumer: str, provider: str, contract_details: Dict[str, Any]) -> str:
...
"""
    ...
    def _format_test_results_table(self, results: Dict[str, Any]) -> str:
        ...
        table = "|| Test Suite || Passed || Failed || Skipped || Total || Success Rate ||\n"
    ...
    def _format_metrics_table(self, metrics: Dict[str, Any]) -> str:
        ...
        for metric, value in metrics.items():
            ...
            status = "✅" if value.get('status') == 'pass' else "❌" if value.get('status') == 'fail' else "⚠️"
...
class ConfluenceReporter:
    ...
    def _generate_recommendations(self, results: Dict[str, Any], metrics: Dict[str, Any]) -> List[str]:
        ...
        for suite, data in results.items():
            ...
            if success_rate < 90:
                recommendations.append(f"Improve test success rate for {suite} (currently {success_rate:.1f}%)")
        
        # Analizar métricas
        if metrics.get('response_time', {}).get('value', 0) > metrics.get('response_time', {}).get('target', 5):
            recommendations.append("Optimize API response times")
        
        if metrics.get('coverage', {}).get('value', 0) < metrics.get('coverage', {}).get('target', 80):
