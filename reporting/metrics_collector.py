...
class MetricsCollector:
    ...
    def stop_collection(self):
        ...
        logger.info(f"Recolección de métricas detenida. Duración: {self.end_time - self.start_time}")
    ...
    def get_overall_metrics(self) -> Dict[str, Any]:
        ...
        average_duration = total_duration / total_tests if total_tests > 0 else 0
        slowest_test = max(self.test_metrics, key=lambda x: x.duration) if self.test_metrics else None
        
        # Métricas de recursos
        memory_peak = max(t.memory_usage for t in self.test_metrics) if self.test_metrics else 0
        cpu_peak = max(t.cpu_usage for t in self.test_metrics) if self.test_metrics else 0
    ...
    def get_suite_metrics(self, suite_name: str) -> Optional[Dict[str, Any]]:
        ...
        suite_metric = next((s for s in self.suite_metrics if s.suite_name == suite_name), None)
        if not suite_metric:
            return None
    ...
    def _export_csv(self) -> str:
        ...
        writer.writerow([
            "test_name", "suite", "status", "duration", "memory_usage", 
            "cpu_usage", "timestamp", "error_message"
        ])
