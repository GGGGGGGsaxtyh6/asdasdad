...
class DashboardGenerator:
    ...
    def __init__(self, metrics_collector: MetricsCollector):
        ...
        self.config = DashboardConfig(title="ECI Testing Dashboard", theme="light")
    ...
    def generate_overview_dashboard(self, output_path: str = "dashboard_overview.html"):
        ...
        if metrics.get("suites"):
            ...
            success_rates = [
                metrics["suites"][suite]["success_rate"] for suite in suite_names
            ]
                ...
                go.Bar(
                    ...
                    marker_color=[
                        (
                            "#28a745"
                            if rate >= 90
                            else "#ffc107" if rate >= 70 else "#dc3545"
                        )
    ...
    def generate_suite_dashboard(self, suite_name: str, output_path: str = None):
        ...
        suite_tests = [
            t for t in self.metrics_collector.test_metrics if t.suite == suite_name
        ]
            ...
            fig.add_trace(
                go.Box(
                    ...
                    marker_color=(
                        "#28a745"
                        if status == "PASSED"
                        else "#dc3545" if status == "FAILED" else "#ffc107"
                    ),
                ),
                row=1,
                col=2,
        ...
        if suite_metrics["failed_tests"] > 0:
            ...
            for test in failed_tests:
                if test.error_message:
                    ...
                    error_type = (
                        test.error_message.split(":")[0]
                        if ":" in test.error_message
                        else "Unknown Error"
                    )
    ...
    def generate_performance_dashboard(
        self, output_path: str = "dashboard_performance.html"
    ):
        ...
        if self.metrics_collector.system_metrics:
            ...
            memory_usage = [
                m.memory_percent for m in self.metrics_collector.system_metrics
            ]
    ...
    def generate_comparison_dashboard(
        self, suites: List[str], output_path: str = "dashboard_comparison.html"
    ):
        ...
        fig.add_trace(
            go.Bar(
                ...
                name="Success Rate %",
                marker_color=[
                    "#28a745" if rate >= 90 else "#ffc107" if rate >= 70 else "#dc3545"
                    for rate in success_rates
                ],
            ),
            row=1,
        ...
        for suite_name in suite_names:
            ...
            if durations:
                fig.add_trace(
                    go.Box(y=durations, name=suite_name, boxpoints="outliers"),
                    row=1,
                    col=2,
                )
        ...
        fig.add_trace(
            go.Bar(
                x=suite_names, y=cpu_peaks, name="CPU Peak %", marker_color="#6f42c1"
            ),
            row=2,
            col=1,
        )
        ...
        fig.add_trace(
            go.Bar(
                x=suite_names, y=total_tests, name="Total Tests", marker_color="#007bff"
            ),
            row=2,
            col=2,
        )
    ...
    def generate_all_dashboards(self, output_dir: str = "dashboards"):
        ...
        for suite_name in suite_names:
            suite_path = os.path.join(
                output_dir, f"{suite_name.lower().replace(' ', '_')}.html"
            )
        ...
        if len(suite_names) > 1:
            ...
            dashboards.append(
                self.generate_comparison_dashboard(list(suite_names), comparison_path)
            )
