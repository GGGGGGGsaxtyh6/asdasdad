...
class APIResponseValidator:
    ...
    def validate_response(self, response: APIResponse) -> ValidationResult:
        ...
        if response.response_time > 5.0:  # 5 segundos
            all_warnings.append(f"Tiempo de respuesta lento: {response.response_time:.2f}s")
...
class ContractValidator:
    ...
    def validate_contract(self, service_name: str, response: APIResponse) -> ValidationResult:
        ...
        if "expected_status" in contract:
            if response.status_code != contract["expected_status"]:
                errors.append(f"Status code esperado {contract['expected_status']}, obtenido {response.status_code}")
...
class PerformanceValidator:
    ...
    def validate_performance(self, response: APIResponse) -> ValidationResult:
        ...
        if response.response_time > self.max_response_time:
            errors.append(f"Tiempo de respuesta excedido: {response.response_time:.2f}s > {self.max_response_time}s")
