...
class ConsolidatedReportGenerator:
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "slowest_tests": [
                {"name": test.name, "suite": test.suite, "duration": test.duration}
...
"""
        ...
        with open(
            self.reports_dir / "consolidated-report.html", "w", encoding="utf-8"
        ) as f:
    ...
    def _generate_suites_table(self, suites: Dict[str, Any]) -> str:
        ...
        for name, data in suites.items():
            status_class = (
                ...
                else "warning" if data["success_rate"] >= 90 else "danger"
            )
            rows.append(
                f"""
                <tr>
    ...
    def _generate_slowest_tests_section(
        self, slowest_tests: List[Dict[str, Any]]
    ) -> str:
    ...
    def _generate_json_report(self, metrics: Dict[str, Any]):
        """Generar reporte JSON"""
        with open(
            self.reports_dir / "consolidated-report.json", "w", encoding="utf-8"
        ) as f:
    ...
    def _generate_recommendations(self, metrics: Dict[str, Any]) -> str:
        ...
        if metrics["overall"]["success_rate"] < 90:
            recommendations.append(
                "Investigate and fix failed tests to improve success rate"
            )

        if metrics["overall"]["failed_tests"] > 0:
            recommendations.append(
                "Review failed tests and update test cases if needed"
            )
